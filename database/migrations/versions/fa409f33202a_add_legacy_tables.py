...
def upgrade() -> None:
    ...
    legacy_exec_overview_path = join(
        dirname(dirname(dirname(abspath(__file__)))),
        "legacy",
        "legacy_exec_overview.csv",
    )
    pd.read_csv(legacy_exec_overview_path).to_sql(
        "legacy_exec_overview", con=op.get_bind(), if_exists="replace", index=False
    )

    legacy_reddit_path = join(
        dirname(dirname(dirname(abspath(__file__)))), "legacy", "legacy_reddit.csv"
    )
    pd.read_csv(legacy_reddit_path).to_sql(
        "legacy_reddit", con=op.get_bind(), if_exists="replace", index=False
    )

    legacy_discord_path = join(
        dirname(dirname(dirname(abspath(__file__)))), "legacy", "legacy_discord.csv"
    )
    pd.read_csv(legacy_discord_path).to_sql(
        "legacy_discord", con=op.get_bind(), if_exists="replace", index=False
    )

    legacy_x_path = join(
        dirname(dirname(dirname(abspath(__file__)))), "legacy", "legacy_x.csv"
    )
    pd.read_csv(legacy_x_path).to_sql(
        "legacy_x", con=op.get_bind(), if_exists="replace", index=False
    )

    legacy_youtube_path = join(
        dirname(dirname(dirname(abspath(__file__)))), "legacy", "legacy_youtube.csv"
    )
    pd.read_csv(legacy_youtube_path).to_sql(
        "legacy_youtube", con=op.get_bind(), if_exists="replace", index=False
    )

    legacy_ios_appstore_path = join(
        dirname(dirname(dirname(abspath(__file__)))),
        "legacy",
        "legacy_ios_appstore.csv",
    )
    pd.read_csv(legacy_ios_appstore_path).to_sql(
        "legacy_ios_appstore", con=op.get_bind(), if_exists="replace", index=False
    )

    legacy_playstore_path = join(
        dirname(dirname(dirname(abspath(__file__)))), "legacy", "legacy_playstore.csv"
    )
    pd.read_csv(legacy_playstore_path).to_sql(
        "legacy_playstore", con=op.get_bind(), if_exists="replace", index=False
    )

    legacy_telegram_radix_dlt_path = join(
        dirname(dirname(dirname(abspath(__file__)))),
        "legacy",
        "legacy_telegram_radix_dlt.csv",
    )
    pd.read_csv(legacy_telegram_radix_dlt_path).to_sql(
        "legacy_telegram_radix_dlt", con=op.get_bind(), if_exists="replace", index=False
    )
    ...
    legacy_network_path = join(
        dirname(dirname(dirname(abspath(__file__)))), "legacy", "legacy_network.csv"
    )
    pd.read_csv(legacy_network_path).to_sql(
        "legacy_network", con=op.get_bind(), if_exists="replace", index=False
    )

    legacy_ecosystem_path = join(
        dirname(dirname(dirname(abspath(__file__)))), "legacy", "legacy_ecosystem.csv"
    )
    pd.read_csv(legacy_ecosystem_path).to_sql(
        "legacy_ecosystem", con=op.get_bind(), if_exists="replace", index=False
    )

    legacy_website_path = join(
        dirname(dirname(dirname(abspath(__file__)))), "legacy", "legacy_website.csv"
    )
    pd.read_csv(legacy_website_path).to_sql(
        "legacy_website", con=op.get_bind(), if_exists="replace", index=False
    )
