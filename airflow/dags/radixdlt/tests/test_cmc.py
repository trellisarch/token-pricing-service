...
class TestProcessCmcPrices:
    ...
    def test_successful_price_retrieval(self, mock_insert_price, mock_cmc_pairs):
        mock_cmc_pairs.split.return_value = ["BTC/USD", "ETH/USD"]
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "data": {
                "BTC": [{"quote": {"USD": {"price": 40000}}}],
                "ETH": [{"quote": {"USD": {"price": 1000}}}],
            }
        }
        with patch.object(requests, "get", return_value=mock_response):
            cmc_prices = process_cmc_prices()
            mock_insert_price.assert_any_call("BTC/USD", 40000, "CMC")
            mock_insert_price.assert_any_call("ETH/USD", 1000, "CMC")
