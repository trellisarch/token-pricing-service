...
def get_telegram_info(user_name, bot_id, combot_id, combot_api_key):
    ...
    try:
        ...
        to_timestamp = int((current_time.replace(hour=23, minute=59, second=59) + timedelta(seconds=1)).timestamp())
        from_timestamp = int((current_time - timedelta(days=6)).replace(hour=0, minute=0, second=0).timestamp())
        ...
        TelegramData.fetch_and_save_data(user_name, bot_response.json()['result'], combot_response.json()[0])
...
def repo_task(user_name, bot_id, combot_id, combot_api_key):
    return PythonOperator(
        ...
        python_callable=lambda: get_telegram_info(user_name, bot_id, combot_id, combot_api_key),
        dag=dag,
    )


telegram_user_task1 = repo_task("radix_dlt", 
                                Config.TELEGRAM_RADIX_DLT_BOT_ID, 
                                Config.TELEGRAM_RADIX_DLT_COMBOT_ID, 
                                Config.TELEGRAM_RADIX_DLT_COMBOT_API_KEY
                                )
telegram_user_task2 = repo_task("RadixDevelopers", 
