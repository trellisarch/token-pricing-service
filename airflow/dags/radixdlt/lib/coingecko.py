...
class GeckoPriceProvider(BasePriceProvider):
        ...
        try:
            ...
            for coin_id in coin_ids:
                if coin_id != "radix":
                    ...
                    coin_pair = f"{COIN_DICT[coin_id]}/XRD"
                    coin_gecko_xrd_price = self.calculate_xrd_quote(
                        coin_gecko_price[coin_id]["usd"],
                        coin_gecko_price["radix"]["usd"],
                    )
                    if coin_pair in Config.STALE_CHECK_PAIRS:
                        ...
                        if last_updated < Config.STALE_PERIOD_SECS:
                            ...
                            self.prices[coin_pair] = coin_gecko_xrd_price
                        else:
                            logging.info(
            ...
            logging.info(str(e))
        logging.info(f"Gecko prices: {self.prices}")
